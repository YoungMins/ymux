...
- [ ] **Step 1: i18n first, so every other file can reference the keys**
...
- [ ] **Step 2: Window-level bindings (`src/main.ts`)**
...
- [ ] **Step 3: Let the keys out of xterm (`src/terminal/TerminalPane.ts`)**
...
- [ ] **Step 4: Let the keys out of a browser pane (`src-tauri/src/embedded_browser.rs`)**
...
- [ ] **Step 5: Help overlay (`src/help/HelpOverlay.ts`)**
...
- [ ] **Step 6: Command palette (`src/palette/commands.ts`)**
...
- [ ] **Step 4: Wire it into `WorkspaceManager`**
...
- [ ] **Step 5: Subscribe in `src/filedock/FileDock.ts`**
...
- [ ] **Step 2: Cross-check the spec's own test list**

Spec §4 names the tests this feature must have. Confirm each exists and passes:

- [ ] vitest — tab node helpers: add / close / switch / active clamping → `src/layout/tabs.test.ts` + the `removePane in a tab group` block in `src/layout/LayoutTree.test.ts`.
- [ ] vitest — closing the last tab unwraps the Tabs node → `unwraps back to a plain pane when one tab is left`.
- [ ] vitest — label derivation → `src/terminal/tabLabel.test.ts`.
- [ ] vitest — tree model with tabs → `buildAgentTree with tabs` in `src/workspace/agentTree.test.ts`.
- [ ] vitest — viewer-tab reuse decision → `src/workspace/viewerTab.test.ts`.
- [ ] Rust — `Tabs` TOML round-trip with several children → `config::model::tests::tabs_node_roundtrips_through_toml`.
- [ ] Rust — process scan reporting a non-agent descendant → `agent_scan::tests::scan_labels_covers_every_pane_with_a_child_process`.
- [ ] Rust — ydir `open-file` message → `yipc::protocol::tests::open_file_event_roundtrips_and_is_readable` + `ydir::dock::tests::open_file_link_relays_a_path_to_the_host_as_an_open_file_event`.
