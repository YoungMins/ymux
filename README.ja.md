...
- **ファイルドック**: `Ctrl+Shift+E` で `ydir` を実行する右側パネルの開閉を
  切り替えます。このパネルは `cd` するたびにアクティブペインの作業ディレクト
  リに追従します。ファイル上で Enter を押すと、ドックに留まらず再利用される
  ビューワータブでそのファイルが開きます。ドック内の yDir は 2 ペインでは
  なく、1 つのファイル一覧とその下に選択中のファイル・フォルダのプレビューを
  表示します。`Tab` でプレビューの表示を切り替えられ、ドックの左端をドラッグ
  すると幅を変えられます。
