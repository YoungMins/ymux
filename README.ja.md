...
- **エージェントセッションの再開**: Claude Code や Codex が動いているペインで
  ymux を再起動すると、古いスクロールバックを表示する代わりに同じ会話を再開
  します (`claude --resume <id> --dangerously-skip-permissions` /
  `codex resume <id>`)。エージェント追跡をオンにしていなくても動作します —
  CLI 自身がディスクに残すセッション記録からセッション id を見つけます。
  ターミナルペインは影響を受けず、今まで通りスクロールバックを復元します。
...
| ペイン | 開き方 | 機能 |
|--------|--------|------|
| **ファイル** | 右クリック → *ここでファイルを表示*、ファイルドック | 閲覧、プレビュー、名前変更、新規作成、コピー/移動、ごみ箱へ削除 |
| **エディター** | 右クリック → *分割: エディターペイン*、ファイル上で Enter | シンタックスハイライト付きテキストエディター — 保存、検索/置換、行へ移動、CRLF/BOM 保持、未保存の変更の確認 |
| **Git** | 右クリック → *ここで Git* | コミットグラフ、ブランチ一覧とチェックアウト、worktree の追加/削除 |
